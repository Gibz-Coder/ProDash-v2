...
const getDateRangeDescription = computed(() => {
    ...
    if (shift === 'ALL' && cutoff !== 'ALL') {
        ...
        if (cutoff === '1ST')
            return `${date} 07:00~11:59 & ${date} 19:00~23:59`;
        if (cutoff === '2ND')
            return `${date} 12:00~15:59 & ${nextDay} 00:00~03:59`;
        if (cutoff === '3RD')
            return `${date} 16:00~18:59 & ${nextDay} 04:00~06:59`;
    }
...
const sortedLots = computed(() => {
    ...
    return lotsCopy.sort((a, b) => {
        ...
        if (
            sortField.value === 'est_endtime' ||
            sortField.value === 'actual_submitted_at' ||
...
const isSearchActive = computed(() => {
    return (
        currentFilters.value.search && currentFilters.value.search.trim() !== ''
    );
});
...
const handleExport = async () => {
    ...
    try {
        ...
        const rows: string[][] = data.lots.map((lot: any) => {
            const machines =
                Array.from({ length: 10 }, (_, i) => lot[`eqp_${i + 1}`])
                    .filter(Boolean)
                    .join(', ') || '-';
            ...
            if (date === '-' || !lot.computed_date) {
                if (lot.status === 'Ongoing' && lot.est_endtime) {
                    ...
                    lot.actual_submitted_at
                ) {
                    // For Submitted lots, use actual_submitted_at
                    const extracted = extractDateShiftCutoff(
                        lot.actual_submitted_at,
                    );
                    date = extracted.date;
                    shift = extracted.shift;
            ...
            return [
                ...
                lot.actual_submitted_at
                    ? formatDateTime(lot.actual_submitted_at)
                    : '-',
                lot.status,
                lot.modified_by || '-',
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <template #filters>
            <div class="flex items-center gap-3">
                ...
                <div
                    class="group relative flex items-center gap-2 rounded-lg border border-border bg-background px-3 py-1.5 shadow-sm"
                ...
                >
                    ...
                    <input
                        ...
                        class="cursor-pointer border-0 bg-transparent text-xs font-semibold text-foreground focus:ring-0 focus:outline-none disabled:cursor-not-allowed"
                    />
                    <!-- Tooltip showing actual datetime range -->
                    <div
                        class="absolute top-full left-0 z-50 mt-1 hidden w-64 rounded-lg border border-border bg-popover p-2 text-xs shadow-lg group-hover:block"
                    >
                ...
                >
                    ...
                    <select
                        ...
                        @change="handleFilterChange"
                        :disabled="isSearchActive ? true : undefined"
                        class="cursor-pointer border-0 bg-transparent pr-6 text-xs font-semibold text-foreground focus:ring-0 focus:outline-none disabled:cursor-not-allowed [&>option]:bg-background [&>option]:text-foreground [&>option]:dark:bg-gray-800 [&>option]:dark:text-gray-100"
                    >
                        <option>ALL</option>
                        <option>DAY</option>
                        <option>NIGHT</option>
                ...
                <!-- WorkType Filter -->
                <div
                    class="flex items-center gap-1.5 rounded-lg border border-border bg-background px-2 py-1.5 shadow-sm"
                    :class="{ 'opacity-50': isSearchActive }"
                >
                    <span
                    ...
                    <select
                        ...
                        @change="handleFilterChange"
                        :disabled="isSearchActive ? true : undefined"
                        class="max-w-[90px] cursor-pointer border-0 bg-transparent pr-4 text-xs font-semibold text-foreground focus:ring-0 focus:outline-none disabled:cursor-not-allowed [&>option]:bg-background [&>option]:text-foreground [&>option]:dark:bg-gray-800 [&>option]:dark:text-gray-100"
                    >
                        <option>ALL</option>
                ...
                <Button
                    variant="outline"
                    size="sm"
                    class="h-8 gap-1.5 rounded-lg px-3 hover:bg-muted"
        ...
        <div class="flex h-full flex-1 flex-col gap-3 p-4">
            ...
            <div class="grid gap-3 md:grid-cols-3 lg:grid-cols-6">
                <div
                    ...
                    :key="index"
                    class="group relative overflow-hidden rounded-xl border-2 border-border shadow-lg transition-all duration-300 hover:-translate-y-0.5 hover:scale-[1.02] hover:shadow-xl dark:border-gray-700"
                    :class="stat.color"
                >
                    <div class="relative p-4">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <div
                                class="rounded-full p-3 ring-1 transition-all duration-300"
                                :class="stat.iconBg"
                            >
            ...
            <!-- Lots Table -->
            <Card
                class="flex-1 overflow-hidden border-2 border-border shadow-lg dark:border-gray-700 dark:bg-gray-900/50"
            >
                    ...
                    >
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex items-center gap-2">
                                <Badge
                                    class="border border-blue-500/30 bg-blue-500/20 px-3 py-1 text-blue-700 dark:text-blue-300"
                                >
                                    <span class="text-xs font-semibold"
                                        >🎯 Target:
                                        {{
                                            formatMillions(
                                                props.badgeStats.targetQty,
                                            )
                                        }}</span
                                    >
                                </Badge>
                                <Badge
                                    class="border border-orange-500/30 bg-orange-500/20 px-3 py-1 text-orange-700 dark:text-orange-300"
                                >
                                    <span class="text-xs font-semibold"
                                        >⏳ Endtime:
                                        {{
                                            formatMillions(
                                                props.badgeStats.endtimeQty,
                                            )
                                        }}</span
                                    >
                                </Badge>
                                <Badge
                                    class="border border-emerald-500/30 bg-emerald-500/20 px-3 py-1 text-emerald-700 dark:text-emerald-300"
                                >
                                    <span class="text-xs font-semibold"
                                        >✅ Submitted:
                                        {{
                                            formatMillions(
                                                props.badgeStats.submittedQty,
                                            )
                                        }}</span
                                    >
                                </Badge>
                                <Badge
                                    class="border border-purple-500/30 bg-purple-500/20 px-3 py-1 text-purple-700 dark:text-purple-300"
                                >
                                    <span class="text-xs font-semibold"
                                        >📊 Achievement:
                                        {{
                                            props.badgeStats.achievement
                                        }}%</span
                                    >
                                </Badge>
                            </div>
                            ...
                            <!-- Search Input -->
                            <div
                                class="flex items-center gap-2 rounded-lg border-2 bg-background px-3 py-1.5 shadow-sm"
                                :class="
                                    isSearchActive
                                        ? 'border-primary'
                                        : 'border-border'
                            ...
                            >
                                ...
                                <Input
                                    ...
                                    class="h-6 w-80 border-0 bg-transparent px-0 text-xs font-semibold text-foreground focus-visible:ring-0 focus-visible:ring-offset-0"
                                />
                                <Button
                                    v-if="isSearchActive"
                                    variant="ghost"
                            ...
                            <Button
                                ...
                                variant="default"
                                size="sm"
                                class="h-9 bg-primary px-4 text-sm font-medium text-primary-foreground shadow-sm hover:bg-primary/90"
                            ...
                            <Button
                                ...
                                variant="default"
                                size="sm"
                                class="h-9 bg-emerald-600 px-4 text-sm font-medium text-white shadow-sm hover:bg-emerald-700"
                ...
                <CardContent class="p-0">
                    <div class="max-h-[calc(100vh-320px)] overflow-auto">
                        <table class="w-full text-xs">
                            <thead
                                class="sticky top-0 z-10 bg-muted/80 dark:bg-gray-800"
                            >
                                <tr
                                    class="border-b-2 border-border dark:border-gray-600"
                                >
                                    <th
                                        class="w-15 p-3 text-left font-semibold"
                                    >
                                        #
                                    </th>
                                    <th
                                        class="w-25 cursor-pointer p-3 text-left font-semibold transition-colors select-none hover:bg-muted/50"
                                    ...
                                    <th
                                        class="w-45 cursor-pointer p-3 text-left font-semibold transition-colors select-none hover:bg-muted/50"
                                    ...
                                    <th
                                        class="w-30 cursor-pointer p-3 text-right font-semibold transition-colors select-none hover:bg-muted/50"
                                    ...
                                    <th
                                        class="cursor-pointer p-3 text-center font-semibold transition-colors select-none hover:bg-muted/50"
                                    ...
                                    <th
                                        class="w-30 cursor-pointer p-3 text-center font-semibold transition-colors select-none hover:bg-muted/50"
                                    ...
                                    >
                                        Est. Endtime
                                        {{ getSortIcon('est_endtime') }}
                                    </th>
                                    <th
                                        class="w-45 cursor-pointer p-3 text-left font-semibold transition-colors select-none hover:bg-muted/50"
                                        @click="
                                            handleSort('actual_submitted_at')
                                        "
                                        title="Click to sort"
                                    >
                                        Submitted Time
                                        {{ getSortIcon('actual_submitted_at') }}
                                    </th>
                                    <th
                                        class="cursor-pointer p-3 text-center font-semibold transition-colors select-none hover:bg-muted/50"
                            ...
                            <tbody>
                                <tr
                                    ...
                                    :key="lot.id"
                                    class="border-b border-border transition-colors hover:bg-muted/50 dark:border-gray-700 dark:hover:bg-gray-800/50"
                                >
                                    ...
                                    <td class="p-3">
                                        <Badge
                                            class="border-blue-500/30 bg-blue-500/20 font-mono font-semibold text-blue-700 dark:text-blue-300"
                                        >
                                            {{ lot.lot_id }}
                                        </Badge>
                                    </td>
                                    <td
                                        class="max-w-32 truncate p-3 text-left font-medium text-foreground"
                                    >
                                        {{ lot.model_15 }}
                                    </td>
                                    ...
                                    <td class="p-3 text-center">
                                        <Badge
                                            :class="
                                                getLipasBadgeClass(lot.lipas_yn)
                                            "
                                    ...
                                    <td
                                        class="p-3 text-emerald-600 dark:text-emerald-400"
                                    >
                                        {{
                                            lot.actual_submitted_at
                                                ? formatDateTime(
            ...
            <DialogContent class="sm:max-w-md">
                ...
                <div class="space-y-4 py-4">
                    <p class="text-sm text-muted-foreground">
                        Select the date range and filters for the data you want
                        to export.
                    </p>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input
                                ...
                                class="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:outline-none"
                            />
                        </div>
