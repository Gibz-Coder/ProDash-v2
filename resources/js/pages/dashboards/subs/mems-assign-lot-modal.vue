...
const handleAssign = async (lot: Lot) => {
    ...
    try {
        ...
        if (props.requestId) {
            const response = await fetch(`/api/lot-request/${props.requestId}/assign`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok && result.success) {
                ...
                showToast('❌ ' + (result.message || 'Failed to assign lot'), 'danger');
            }
        } else {
            ...
            if (response.ok && result.success) {
                ...
                showToast('❌ ' + (result.message || 'Failed to submit lot request'), 'danger');
            }
        }
    } catch (error) {
...
const useMyId = async () => {
    ...
    if (!currentUser) {
        ...
        showToast('⚠️ User session not loaded - please refresh the page', 'warning');
        return;
    }
...
<template>
    <Dialog :open="open" @update:open="handleClose">
        <DialogContent class="!max-w-[1200px] max-h-[90vh] overflow-hidden flex flex-col p-0 gap-0">
            <!-- Header -->
            <DialogHeader class="px-6 py-5 border-b border-border/50 bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-600 dark:from-blue-700 dark:via-blue-800 dark:to-indigo-800">
                <DialogTitle class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-2.5 flex-wrap">
                            <h2 class="text-xl font-bold text-white drop-shadow-sm">Lot Assignment for Machine</h2>
                            ...
                            <span v-if="equipmentArea" class="inline-flex items-center px-3.5 py-1.5 rounded-lg text-sm font-bold bg-white/95 text-green-700 shadow-md ring-1 ring-white/50">
                                {{ equipmentArea }}
                            </span>
                            <!-- Rack Badge with Select Function -->
                            <div
                                v-if="mcRack"
                                class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-sm font-bold bg-white/95 text-orange-700 shadow-md ring-1 ring-white/50"
                            >
                                <span class="text-xs">📍</span>
                                <span>RACK: <span class="rack-text">{{ mcRack }}</span></span>
                                <button
                                    @click="selectRackText"
                                    class="text-xs opacity-70 hover:opacity-100 hover:scale-125 transition-all duration-200 cursor-pointer p-1"
                            ...
                            <span 
                                class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-sm font-bold shadow-md ring-1 ring-white/50"
                                :class="[machineStatus.bgClass, machineStatus.textClass]"
                            >
                            ...
                            <!-- Time Info Badge -->
                            <span 
                                class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-bold shadow-md ring-1 ring-white/50"
                ...
                <DialogDescription class="sr-only">Request a lot assignment for equipment {{ equipmentNo }}</DialogDescription>
            </DialogHeader>

            <!-- Filter Section -->
            <div class="px-6 py-4 bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50 dark:from-slate-900 dark:via-blue-950/20 dark:to-slate-900 border-b border-border/60">
                <div class="grid grid-cols-12 gap-3 items-end">
                    
                    <!-- Employee ID Filter -->
                    <div class="col-span-3 space-y-1.5">
                        <label class="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wide flex items-center gap-1.5">
                        ...
                        <div class="flex gap-1.5">
                            <Input
                                ...
                                class="flex-1 h-10 text-sm shadow-sm transition-all duration-200"
                                :class="{ 
                                    ...
                                    'border-slate-300 dark:border-slate-600 focus:border-blue-500 focus:ring-blue-500/20': !employeeValidationError && !employeeName
                                }"
                            ...
                            <Button 
                                ...
                                class="h-10 px-3 font-medium shadow-sm hover:shadow transition-all duration-200 shrink-0"
                                :class="{
                                    'bg-green-50 border-green-400 text-green-700 hover:bg-green-100 dark:bg-green-950/30 dark:border-green-600 dark:text-green-400': employeeName,
                                    'bg-white dark:bg-slate-800': !employeeName
                                }"
                            >
                                <span v-if="isValidatingEmployee" class="animate-spin text-sm">⟳</span>
                                <span v-else-if="employeeName" class="text-sm">✓</span>
                    ...
                    <div class="col-span-3 space-y-1.5">
                        ...
                        <Input
                            ...
                            class="h-10 text-sm border-slate-300 dark:border-slate-600 focus:border-blue-500 focus:ring-blue-500/20 shadow-sm"
                    ...
                    <div class="col-span-2 space-y-1.5">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger as-child>
                                <Button variant="outline" size="sm" class="w-full justify-between h-10 text-xs font-medium shadow-sm hover:shadow transition-all duration-200 border-slate-300 dark:border-slate-600">
                                    <span class="truncate">{{ wipStatusFilter === 'ALL' ? 'All' : wipStatusFilter }}</span>
                                    <ChevronDown class="ml-1.5 h-3.5 w-3.5 shrink-0 opacity-50" />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="start" class="w-56 max-h-64 overflow-y-auto">
                                <DropdownMenuItem @click="wipStatusFilter = 'ALL'; handleSearch()" class="cursor-pointer">
                    ...
                    <div class="col-span-2 space-y-1.5">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger as-child>
                                <Button variant="outline" size="sm" class="w-full justify-between h-10 text-xs font-medium shadow-sm hover:shadow transition-all duration-200 border-slate-300 dark:border-slate-600">
                                    <span class="truncate">{{ worktypeFilter === 'ALL' ? 'All' : worktypeFilter }}</span>
                                    <ChevronDown class="ml-1.5 h-3.5 w-3.5 shrink-0 opacity-50" />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="start" class="w-48 max-h-64 overflow-y-auto">
                                <DropdownMenuItem @click="worktypeFilter = 'ALL'; handleSearch()" class="cursor-pointer">
                    ...
                    <div class="col-span-2">
                        <Button 
                            ...
                            @click="handleSearch" 
                            :disabled="isLoading" 
                            class="w-full h-10 font-semibold shadow-md hover:shadow-lg transition-all duration-200 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 mt-[26px]"
                        >
                            <span v-if="isLoading" class="flex items-center gap-2 text-xs">
                ...
                <div v-if="employeeName || employeeValidationError" class="mt-3">
                    <div v-if="employeeName" class="flex items-center gap-3">
                        <div class="flex items-center gap-2 px-3 py-2 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-lg">
                            <span class="text-green-600 dark:text-green-400 text-xs">✓</span>
                            <span class="text-xs font-medium text-green-700 dark:text-green-300">Requestor: {{ employeeName }}</span>
                        </div>
                        <label class="flex items-center gap-2 px-3 py-2 bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800 rounded-lg cursor-pointer hover:bg-purple-100 dark:hover:bg-purple-900/40 transition-colors">
                            <input 
                                type="checkbox" 
                                v-model="keepRequestor"
                                class="w-4 h-4 text-purple-600 bg-white border-purple-300 rounded focus:ring-purple-500 focus:ring-2 cursor-pointer"
                            />
                            <span class="text-xs font-medium text-purple-700 dark:text-purple-300">Keep for next request</span>
                        </label>
                        <button
                            @click="clearRequestor"
                            class="flex items-center justify-center w-8 h-8 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors"
                            title="Clear requestor"
                        >
                            <span class="text-red-600 dark:text-red-400 text-sm font-bold">✕</span>
                        </button>
                    </div>
                    <div v-else-if="employeeValidationError" class="flex items-center gap-2 px-3 py-2 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg w-fit">
                        <span class="text-red-600 dark:text-red-400 text-xs">✗</span>
                        <span class="text-xs font-medium text-red-700 dark:text-red-300">{{ employeeValidationError }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-1 flex flex-col overflow-hidden bg-background">
                ...
                <div v-else class="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div class="px-5 pt-4 pb-2 bg-background border-b border-border shrink-0">
                        <div class="grid grid-cols-[0.9fr_1.6fr_0.9fr_0.7fr_0.5fr_0.9fr_1.3fr_0.5fr_0.5fr_1.1fr_1.1fr] gap-2 px-3 py-2 bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg border border-blue-600 text-xs font-semibold text-white shadow-sm">
                            <div class="flex items-center gap-1 cursor-pointer hover:text-blue-100 transition-colors" @click="handleSort('lot_no')">
                                <span>Lot No.</span>
                                <ArrowUpDown v-if="sortColumn !== 'lot_no'" class="h-3 w-3 opacity-50" />
                                <ArrowUp v-else-if="sortDirection === 'asc'" class="h-3 w-3" />
                                <ArrowDown v-else class="h-3 w-3" />
                            </div>
                            <div class="flex items-center gap-1 cursor-pointer hover:text-blue-100 transition-colors" @click="handleSort('lot_model')">
                                <span>Model</span>
                                <ArrowUpDown v-if="sortColumn !== 'lot_model'" class="h-3 w-3 opacity-50" />
                                <ArrowUp v-else-if="sortDirection === 'asc'" class="h-3 w-3" />
                                <ArrowDown v-else class="h-3 w-3" />
                            </div>
                            <div class="flex items-center justify-end gap-1 cursor-pointer hover:text-blue-100 transition-colors" @click="handleSort('lot_qty')">
                                <span>Quantity</span>
                                <ArrowUpDown v-if="sortColumn !== 'lot_qty'" class="h-3 w-3 opacity-50" />
                                <ArrowUp v-else-if="sortDirection === 'asc'" class="h-3 w-3" />
                                <ArrowDown v-else class="h-3 w-3" />
                            </div>
                            <div class="flex items-center justify-center gap-1 cursor-pointer hover:text-blue-100 transition-colors" @click="handleSort('qty_class')">
                                <span>Qty Class</span>
                                <ArrowUpDown v-if="sortColumn !== 'qty_class'" class="h-3 w-3 opacity-50" />
                                <ArrowUp v-else-if="sortDirection === 'asc'" class="h-3 w-3" />
                                <ArrowDown v-else class="h-3 w-3" />
                            </div>
                            <div class="flex items-center justify-center gap-1 cursor-pointer hover:text-blue-100 transition-colors" @click="handleSort('stagnant_tat')">
                                <span>TAT</span>
                                <ArrowUpDown v-if="sortColumn !== 'stagnant_tat'" class="h-3 w-3 opacity-50" />
                                <ArrowUp v-else-if="sortDirection === 'asc'" class="h-3 w-3" />
                                <ArrowDown v-else class="h-3 w-3" />
                            </div>
                            <div class="flex items-center justify-center gap-1 cursor-pointer hover:text-blue-100 transition-colors" @click="handleSort('work_type')">
                                <span>Work Type</span>
                                <ArrowUpDown v-if="sortColumn !== 'work_type'" class="h-3 w-3 opacity-50" />
                                <ArrowUp v-else-if="sortDirection === 'asc'" class="h-3 w-3" />
                                <ArrowDown v-else class="h-3 w-3" />
                            </div>
                            <div class="flex items-center justify-center gap-1 cursor-pointer hover:text-blue-100 transition-colors" @click="handleSort('wip_status')">
                                <span>WIP Status</span>
                                <ArrowUpDown v-if="sortColumn !== 'wip_status'" class="h-3 w-3 opacity-50" />
                                <ArrowUp v-else-if="sortDirection === 'asc'" class="h-3 w-3" />
                                <ArrowDown v-else class="h-3 w-3" />
                            </div>
                            <div class="flex items-center justify-center gap-1 cursor-pointer hover:text-blue-100 transition-colors" @click="handleSort('auto_yn')">
                                <span>Auto</span>
                                <ArrowUpDown v-if="sortColumn !== 'auto_yn'" class="h-3 w-3 opacity-50" />
                                <ArrowUp v-else-if="sortDirection === 'asc'" class="h-3 w-3" />
                                <ArrowDown v-else class="h-3 w-3" />
                            </div>
                            <div class="flex items-center justify-center gap-1 cursor-pointer hover:text-blue-100 transition-colors" @click="handleSort('lipas_yn')">
                                <span>Lipas</span>
                                <ArrowUpDown v-if="sortColumn !== 'lipas_yn'" class="h-3 w-3 opacity-50" />
                                <ArrowUp v-else-if="sortDirection === 'asc'" class="h-3 w-3" />
                                <ArrowDown v-else class="h-3 w-3" />
                            </div>
                            <div class="flex items-center justify-center gap-1 cursor-pointer hover:text-blue-100 transition-colors" @click="handleSort('lot_location')">
                                <span>Location</span>
                    ...
                    <div class="flex-1 overflow-y-auto px-5 pb-4">
                        <div class="space-y-1 pt-2">
                            <div
                                ...
                                :key="lot.lot_no"
                                class="grid grid-cols-[0.9fr_1.6fr_0.9fr_0.7fr_0.5fr_0.9fr_1.3fr_0.5fr_0.5fr_1.1fr_1.1fr] gap-2 p-2 border border-border rounded-lg hover:bg-accent/50 transition-colors items-center"
                            >
                                <div class="font-semibold text-xs text-foreground truncate" :title="lot.lot_no">
                                    {{ lot.lot_no }}
                                </div>
                                <div class="text-xs truncate text-muted-foreground" :title="lot.lot_model">
                                    {{ lot.lot_model }}
                                </div>
                                ...
                                <div class="text-xs text-center truncate" :title="lot.work_type">
                                    {{ lot.work_type }}
                                </div>
                                <div class="text-xs text-center">
                                ...
                                <div class="text-xs text-center truncate" :title="lot.lot_location">
                                    {{ lot.lot_location || '-' }}
                                </div>
                                <div class="text-center">
            ...
            <!-- Footer -->
            <div class="px-6 py-4 border-t border-border bg-gradient-to-br from-slate-50 via-slate-100/50 to-slate-50 dark:from-slate-900 dark:via-slate-800/50 dark:to-slate-900 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="flex items-center gap-2 px-3 py-1.5 bg-blue-100 dark:bg-blue-900/30 rounded-lg border border-blue-200 dark:border-blue-800">
                        <span class="text-blue-600 dark:text-blue-400 text-sm">📊</span>
                        <span class="text-sm font-semibold text-blue-700 dark:text-blue-300">
                            {{ availableLots.length }} lot(s) displayed
                            <span v-if="totalAvailable > availableLots.length" class="text-xs opacity-75">
                                ({{ totalAvailable }} total)
                            </span>
                        </span>
                    </div>
                    <div v-if="isLimitReached" class="flex items-center gap-2 px-3 py-1.5 bg-amber-100 dark:bg-amber-900/30 rounded-lg border border-amber-200 dark:border-amber-800">
                        ...
                        <span class="text-xs font-medium text-amber-700 dark:text-amber-300">
                            Showing first 1000 lots. Use filters to narrow results.
                        </span>
                    </div>
                </div>
                <Button variant="outline" @click="handleClose" class="px-6 font-medium shadow-sm hover:shadow transition-all duration-200">
                    Cancel
                </Button>
            </div>
        </DialogContent>
