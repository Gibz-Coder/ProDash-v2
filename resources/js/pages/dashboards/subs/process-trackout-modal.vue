...
const handleSubmit = async () => {
    ...
    if (!confirm('This will add the new Process Trackout data to the database. Continue?')) {
        return;
    }
    ...
    try {
        router.post('/process-trackout/update', {
            raw_data: rawData.value,
        }, {
            onSuccess: () => {
                showToast('Process Trackout data added successfully!', 'success');
                closeModal();
            },
            onError: (errors) => {
                console.error('Update error:', errors);
                const errorMessage = errors.raw_data || 'Failed to update Process Trackout data. Please try again.';
        ...
        console.error('Error updating Process Trackout:', error);
        isUpdating.value = false;
    }
};
</script>
            ...
            <DialogHeader>
                <div class="flex items-center justify-between gap-4">
                    <DialogTitle class="text-2xl font-bold flex items-center gap-2">
                        ...
                        Process Trackout Data Import
                    </DialogTitle>
                </div>
                
                <DialogDescription>
                    Copy Process Trackout data from Excel and paste it below to update the database
                </DialogDescription>
            </DialogHeader>
                ...
                <div class="flex-1 flex flex-col overflow-hidden space-y-2">
                    <div class="flex items-center justify-between">
                        <Label for="raw_data" class="text-sm font-semibold">
                            ...
                            <span class="text-xs text-muted-foreground font-normal ml-2">MOS40120 - Trackout</span>
                        </Label>
                ...
                <DialogFooter class="border-t border-border pt-4 mt-0 flex-shrink-0">
                    ...
                    >
                        ...
                        <span v-else class="flex items-center gap-2">
                            <span>🔄</span>
                            <span>Update Process Trackout Data</span>
                        </span>
                    </Button>
                </DialogFooter>
            </form>
