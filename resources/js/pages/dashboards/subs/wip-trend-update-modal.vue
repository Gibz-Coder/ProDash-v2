...
watch(rawData, (newValue) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (i === 0 || (validRows === 0 && line.includes('lot_id'))) {
            if (isFirstDataRow && i > 0) validRows = 0; // Reset if we found header late
            continue;
        }
        
        // Count as valid row if it has enough columns (at least 10)
        if (parts.length >= 10) {
            validRows++;
            if (isFirstDataRow) {
...
const downloadTemplate = () => {
    window.location.href = '/wip-trend/download-template';
};
...
const handleSubmit = async () => {
    ...
    if (!confirm('This will add the new WIP Trend data to the database. Continue?')) {
        return;
    }
    ...
    try {
        router.post('/wip-trend/update', {
            raw_data: rawData.value,
        }, {
            onSuccess: () => {
                showToast('WIP Trend data updated successfully!', 'success');
                closeModal();
            },
            onError: (errors) => {
                console.error('Update error:', errors);
                const errorMessage = errors.raw_data || 'Failed to update WIP Trend data. Please try again.';
        ...
        console.error('Error updating WIP Trend:', error);
        isUpdating.value = false;
    }
};
</script>
            ...
            <DialogHeader>
                <div class="flex items-center justify-between gap-4">
                    <DialogTitle class="text-2xl font-bold flex items-center gap-2">
                        ...
                        WIP Trend Data Import
                    </DialogTitle>
                </div>
                
                <DialogDescription>
                    Copy WIP Trend data from Excel and paste it below to update the database
                </DialogDescription>
            </DialogHeader>
                ...
                <div class="flex-1 flex flex-col overflow-hidden space-y-2">
                    <div class="flex items-center justify-between">
                        <Label for="raw_data" class="text-sm font-semibold">
                            ...
                            <span class="text-xs text-muted-foreground font-normal ml-2">MOS40284</span>
                        </Label>
                ...
                <DialogFooter class="border-t border-border pt-4 mt-0 flex-shrink-0">
                    ...
                    >
                        ...
                        <span v-else class="flex items-center gap-2">
                            <span>🔄</span>
                            <span>Update WIP Trend Data</span>
                        </span>
                    </Button>
                </DialogFooter>
            </form>
