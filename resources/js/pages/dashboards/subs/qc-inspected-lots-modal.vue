...
const props = defineProps<{ open: boolean; editRecord?: QcInspectionRecord | null }>();
...
const defectCache = ref<Record<string, { defect_flow: string; defect_class: string }>>({});
let defectTimer: ReturnType<typeof setTimeout> | null = null;

function emptyForm(): FormState {
    return {
        lot_id: '', model: '', lot_qty: null, lipas_yn: '', work_type: '',
        inspection_times: null, inspection_spl: null, inspected_bin: '',
        qc_result: '', inspected_bin_result: [], qc_ng_part: [],
        inspection_result: [], defect_codes: [], defect_input: '',
        defect_flow: '', final_decision: 'Pending', remarks: '', _loading: false,
    };
}
...
watch(() => props.open, (val) => {
    ...
    if (props.editRecord) {
        ...
        form.value = {
            ...
            qc_result: props.editRecord.inspection_result?.includes('OK') ? 'OK' : (props.editRecord.inspection_result ? 'NG' : ''),
            inspected_bin_result: [],
            qc_ng_part: props.editRecord.inspection_result
                    ? props.editRecord.inspection_result.split(',').map((s) => s.trim()).filter((s) => ['Main','RR','LY'].includes(s))
                    : [],
            inspection_result: props.editRecord.inspection_result
                    ? props.editRecord.inspection_result.split(',').map((s) => s.trim()).filter(Boolean)
                    : [],
            defect_codes: props.editRecord.defect_code
                ? props.editRecord.defect_code.split(',').map((s) => s.trim()).filter(Boolean)
                : [],
            defect_input: '',
...
async function lookupLot() {
    ...
    try {
        const { data } = await axios.get('/api/endline-delay/lot-lookup', { params: { lot_id: lotId } });
        if (data) {
...
async function onDefectInput(val: string) {
    ...
    if (!val.trim()) { defectSuggestions.value = []; defectOpen.value = false; return; }
    defectTimer = setTimeout(async () => {
        const { data } = await axios.get<DefectOption[]>('/api/endline-delay/defect-codes', { params: { q: val } });
...
function onDefectRawInput(e: Event) {
    form.value.defect_input = (e.target as HTMLInputElement).value.toUpperCase();
    onDefectInput(form.value.defect_input);
}

function resolveDefectFlow() {
    if (!form.value.defect_codes.length) { form.value.defect_flow = ''; return; }
    const priority = form.value.defect_codes.find((c) => defectCache.value[c]?.defect_class === 'QC Analysis');
    const key = priority ?? form.value.defect_codes.find((c) => defectCache.value[c]);
    form.value.defect_flow = key ? (defectCache.value[key]?.defect_flow ?? '') : '';
}

function selectDefect(opt: DefectOption) {
    if (!form.value.defect_codes.includes(opt.defect_code)) form.value.defect_codes.push(opt.defect_code);
    defectCache.value[opt.defect_code] = { defect_flow: opt.defect_flow, defect_class: opt.defect_class };
...
function onDefectKeydown(e: KeyboardEvent) {
    if (e.key === ',' || e.key === 'Enter') {
        ...
        const exact = defectSuggestions.value.find((d) => d.defect_code === val);
        if (exact) selectDefect(exact);
        else { form.value.defect_input = ''; defectOpen.value = false; }
    } else if (e.key === 'Backspace' && form.value.defect_input === '' && form.value.defect_codes.length) {
...
function toggleInspectionResult(opt: string) {
    ...
    if (opt === 'OK') {
        form.value.inspection_result = form.value.inspection_result.includes('OK') ? [] : ['OK'];
        if (form.value.inspection_result.includes('OK')) { form.value.defect_codes = []; form.value.defect_flow = ''; }
        return;
    }
    const without = form.value.inspection_result.filter((v) => v !== 'OK');
    form.value.inspection_result = without.includes(opt) ? without.filter((v) => v !== opt) : [...without, opt];
}
...
function toggleInspectedBin(opt: string) {
    ...
    form.value.inspected_bin_result = form.value.inspected_bin_result.includes(opt)
...
watch(() => form.value.inspection_spl, (val) => {
...
watch(() => form.value.lot_qty, (val) => {
    lotQtyDisplay.value = val != null ? val.toLocaleString() : '';
}, { immediate: true });

function onInspTimesInput() { delete errors.value.inspection_times; }
...
function buildAutoRemarks(): string {
    ...
    if (form.value.qc_result === 'OK') {
        ...
        const t = form.value.inspection_times != null ? `-${form.value.inspection_times}` : '';
...
function validate(): boolean {
    ...
    if (form.value.inspected_bin_result.length === 0) errors.value.inspected_bin_result = 'Select at least one inspected bin';
    if (form.value.qc_result === 'NG' && form.value.qc_ng_part.length === 0) errors.value.qc_ng_part = 'Select at least one NG bin';
    if (form.value.inspection_times == null || form.value.inspection_times < 1) errors.value.inspection_times = 'Required';
    if (form.value.inspection_spl == null || form.value.inspection_spl < 0) errors.value.inspection_spl = 'Required';
    if (form.value.qc_result === 'NG' && form.value.defect_codes.length === 0) errors.value.defect_codes = 'Defect code required for NG';
...
function toPayload() {
    ...
    return {
        ...
        defect_code: form.value.defect_codes.length ? form.value.defect_codes.join(', ') : null,
...
async function save() {
    ...
    try {
        ...
        if (err.response?.status === 422) {
            ...
            if (raw) Object.keys(raw).forEach((k) => (errors.value[k] = raw[k][0]));
            else if (err.response.data?.message) errors.value.lot_id = err.response.data.message;
        }
    } finally {
...
function showToast(message: string, type: 'success' | 'danger' | 'info' | 'warning' = 'info') {
    const bgColors = { success: 'bg-green-600', danger: 'bg-red-600', info: 'bg-blue-600', warning: 'bg-amber-600' };
...
function onAltS(e: KeyboardEvent) {
    if (e.altKey && (e.key === 's' || e.key === 'S') && props.open) { e.preventDefault(); save(); }
}
...
<template>
    <Dialog :open="open" @update:open="(v: boolean) => emit('update:open', v)">
        <DialogContent class="flex !max-h-[95vh] !w-[820px] !max-w-[820px] flex-col gap-0 overflow-hidden p-0">

            <!-- Header -->
            <DialogHeader class="relative overflow-hidden rounded-t-lg border-b border-border/50 px-6 py-4">
                <div class="absolute inset-0 bg-gradient-to-br from-teal-500/10 via-transparent to-primary/5 pointer-events-none"></div>
                <div class="relative flex items-center gap-3">
                    <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-teal-500/15 ring-1 ring-teal-500/30">
                        <ClipboardList class="h-4 w-4 text-teal-600 dark:text-teal-400" />
                    </div>
                    <div>
                        <DialogTitle class="text-base font-bold text-foreground leading-tight">
                            {{ isEditMode ? 'Edit QC Inspection' : 'QC Inspected Lot Entry' }}
                        </DialogTitle>
                        <DialogDescription class="text-xs text-muted-foreground mt-0.5">
                            {{ isEditMode ? 'Update inspection record' : 'Record a single QC inspected lot' }}
                        </DialogDescription>
                    </div>
                </div>
            </DialogHeader>
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div class="grid grid-cols-12 gap-3 items-end">
                    ...
                    <div class="col-span-2">
                        ...
                        <div class="relative">
                            <input
                                ...
                                maxlength="7"
                                placeholder=""
                                class="h-9 w-full rounded-lg border bg-background pr-9 pl-3 text-sm font-mono font-semibold uppercase tracking-wider placeholder:font-normal placeholder:normal-case placeholder:tracking-normal placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-teal-500/20"
                                :class="errors.lot_id ? 'border-destructive' : 'border-input focus:border-teal-500'"
                                @input="onLotInput"
                                @blur="lookupLot"
                                @keydown.enter.prevent="lookupLot"
                            />
                            <span class="pointer-events-none absolute inset-y-0 right-2.5 flex items-center">
                                <Loader2 v-if="form._loading" class="h-4 w-4 animate-spin text-teal-500" />
                                <Search v-else class="h-3.5 w-3.5 text-muted-foreground/40" />
                            </span>
                        </div>
                        <p v-if="errors.lot_id" class="mt-1 text-[10px] text-destructive">{{ errors.lot_id }}</p>
                    </div>

                    <!-- Scan Only toggle — 1/12 -->
                    <div class="col-span-1 flex flex-col items-center justify-end pb-1 gap-1">
                        <span class="text-[10px] font-semibold text-muted-foreground leading-tight text-center">Scan<br>Only</span>
                        <button
                            type="button"
                            class="relative h-5 w-9 rounded-full transition-colors duration-200 focus:outline-none"
                            :class="scanOnly ? 'bg-amber-500' : 'bg-slate-300 dark:bg-slate-600'"
                        ...
                        >
                            <span
                                ...
                                :class="scanOnly ? 'translate-x-4' : 'translate-x-0'"
                            ></span>
                        </button>
                    </div>
                    ...
                    <div class="col-span-1">
                        <label class="mb-1 block text-xs font-semibold text-foreground">LIPAS</label>
                        <select
                            v-if="scanOnly"
                            v-model="form.lipas_yn"
                            class="h-9 w-full rounded-lg border border-amber-400 bg-background px-2 text-sm font-semibold focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                        >
                            <option value="">—</option>
                            <option value="Y">Y</option>
                            <option value="N">N</option>
                        </select>
                        <div v-else class="flex h-9 items-center justify-center rounded-lg border border-input bg-muted/40 px-2 text-sm gap-1.5">
                            <template v-if="form.lipas_yn">
                                <span class="h-2 w-2 rounded-full shrink-0" :class="form.lipas_yn === 'Y' ? 'bg-emerald-500' : 'bg-slate-400'"></span>
                                <span class="text-xs font-bold" :class="form.lipas_yn === 'Y' ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'">{{ form.lipas_yn }}</span>
                            </template>
                    ...
                    <div class="col-span-4">
                        <label class="mb-1 block text-xs font-semibold text-foreground">Model</label>
                        <input
                            v-if="scanOnly"
                            v-model="form.model"
                            type="text"
                            placeholder="Enter model..."
                            class="h-9 w-full rounded-lg border border-amber-400 bg-background px-3 text-sm placeholder:text-muted-foreground/50 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                        />
                        <div v-else class="flex h-9 items-center rounded-lg border border-input bg-muted/40 px-3 text-sm">
                            <span v-if="form.model" class="truncate text-foreground">{{ form.model }}</span>
                            <span v-else class="text-muted-foreground/40 text-xs">—</span>
                        </div>
                    </div>
                    ...
                    <!-- Qty — 2/12 -->
                    <div class="col-span-2">
                        <label class="mb-1 block text-xs font-semibold text-foreground">Qty</label>
                        <input
                            v-if="scanOnly"
                            :value="lotQtyDisplay"
                            type="text"
                            inputmode="numeric"
                            placeholder="0"
                            class="h-9 w-full rounded-lg border border-amber-400 bg-background px-3 text-sm font-medium tabular-nums focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                            @input="onLotQtyInput"
                        />
                        <div v-else class="flex h-9 items-center rounded-lg border border-input bg-muted/40 px-3 text-sm font-medium">
                            <span v-if="form.lot_qty != null" class="text-foreground">{{ form.lot_qty.toLocaleString() }}</span>
                            <span v-else class="text-muted-foreground/40 text-xs">—</span>
                        </div>
                    </div>
                    ...
                    <!-- Work Type — 2/12 -->
                    <div class="col-span-2">
                        <label class="mb-1 block text-xs font-semibold text-foreground">Work Type</label>
                        <select
                            v-if="scanOnly"
                            v-model="form.work_type"
                            class="h-9 w-full rounded-lg border border-amber-400 bg-background px-3 text-sm text-foreground focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                        >
                            <option value="">—</option>
                        ...
                        <div v-else class="flex h-9 items-center rounded-lg border border-input bg-muted/40 px-3 text-sm">
                            <span v-if="form.work_type" class="truncate text-foreground">{{ form.work_type }}</span>
                            <span v-else class="text-muted-foreground/40 text-xs">—</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <p v-if="errors.qc_result" class="mt-1 text-[10px] text-destructive">{{ errors.qc_result }}</p>
                    </div>

                    <!-- Inspected Bin: Main / RR / LY multi -->
                    <div>
                        <label class="mb-2 block text-xs font-semibold text-foreground">Inspected Bin</label>
                        <div class="grid grid-cols-3 gap-2">                            <button
                                ...
                                :class="form.inspected_bin_result.includes(opt)
                                    ? opt === 'Main' ? 'border-blue-500 bg-blue-500 text-white shadow-md shadow-blue-500/30'
                                    : opt === 'RR'   ? 'border-orange-500 bg-orange-500 text-white shadow-md shadow-orange-500/30'
                        ...
                        <p v-if="errors.inspected_bin_result" class="mt-1 text-[10px] text-destructive">{{ errors.inspected_bin_result }}</p>
                    </div>
                    ...
                    <div>
                        <label class="mb-2 block text-xs font-semibold text-foreground">
                            QC NG Bin
                            <span v-if="form.qc_result === 'NG'" class="text-destructive">*</span>
                        </label>
                        <div class="grid grid-cols-3 gap-2">
                            <button
                                ...
                                :class="form.qc_ng_part.includes(opt)
                                    ? opt === 'Main' ? 'border-red-500 bg-red-500 text-white shadow-md shadow-red-500/30'
                                    : opt === 'RR'   ? 'border-orange-500 bg-orange-500 text-white shadow-md shadow-orange-500/30'
                        ...
                        <p v-if="errors.qc_ng_part" class="mt-1 text-[10px] text-destructive">{{ errors.qc_ng_part }}</p>
                    </div>
                ...
                <div class="grid grid-cols-12 gap-3 items-start">
                    ...
                    <div class="col-span-2">
                        ...
                        <input
                            ...
                            type="number" min="1" max="99" maxlength="2" placeholder="—"
                            class="h-9 w-full rounded-lg border bg-background px-3 text-sm text-center font-semibold focus:outline-none focus:ring-2 focus:ring-teal-500/20"
                            :class="errors.inspection_times ? 'border-destructive' : 'border-input focus:border-teal-500'"
                            @input="onInspTimesInput"
                        />
                        <p v-if="errors.inspection_times" class="mt-1 text-[10px] text-destructive">{{ errors.inspection_times }}</p>
                    </div>

                    <!-- Scan SPL Qty — narrow (5 digits) -->
                    <div class="col-span-2">
                        <label class="mb-1 block text-xs font-semibold text-foreground">SPL Qty <span class="text-destructive">*</span></label>
                        <input
                            :value="splQtyDisplay"
                            type="text"
                            inputmode="numeric"
                            placeholder="—"
                            class="h-9 w-full rounded-lg border bg-background px-3 text-sm text-center font-semibold tabular-nums focus:outline-none focus:ring-2 focus:ring-teal-500/20"
                            :class="errors.inspection_spl ? 'border-destructive' : 'border-input focus:border-teal-500'"
                            @input="onSplQtyInput"
                        />
                        <p v-if="errors.inspection_spl" class="mt-1 text-[10px] text-destructive">{{ errors.inspection_spl }}</p>
                    </div>

                    <!-- Defect Code(s) — multi tag input, wide -->
                    <div class="col-span-6">
                        <label class="mb-1 block text-xs font-semibold text-foreground">Defect Code(s)</label>
                        <div class="relative">
                            <div
                                ...
                                :class="errors.defect_codes ? 'border-destructive' : 'border-input focus-within:border-teal-500'"
                            >
                                <span
                                ...
                                >
                                    {{ code }}
                                    <button type="button" class="leading-none text-red-500/60 hover:text-red-600" @mousedown.prevent="removeDefectTag(code)">×</button>
                                </span>
                                <input
                                    :value="form.defect_input"
                                    type="text"
                                    class="min-w-[80px] flex-1 bg-transparent text-sm uppercase outline-none placeholder:font-normal placeholder:normal-case placeholder:text-muted-foreground/50"
                            ...
                            >
                                ...
                                >
                                    <span class="w-16 shrink-0 font-bold text-foreground">{{ opt.defect_code }}</span>
                                    <span class="truncate text-muted-foreground">{{ opt.defect_name }}</span>
                                    <span class="ml-auto shrink-0 rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">{{ opt.defect_class }}</span>
                                </li>
                            </ul>
                            <div
                                v-else-if="defectOpen && !defectSuggestions.length && form.defect_input.trim()"
                            ...
                            >No matching defect codes found</div>
                        </div>
                        <p v-if="errors.defect_codes" class="mt-1 text-[10px] text-destructive">{{ errors.defect_codes }}</p>
                    </div>

                    <!-- Defect Flow — auto badge -->
                    <div class="col-span-2">
                        <label class="mb-1 block text-xs font-semibold text-foreground">Defect Flow</label>
                        <div class="flex h-9 items-center rounded-lg border border-input bg-muted/40 px-3 text-sm">
                            <span
                                v-if="form.defect_flow"
                                class="inline-flex items-center rounded-md px-2 py-0.5 text-xs font-semibold ring-1 ring-inset"
                                :class="form.defect_flow === 'QC Analysis'
                            ...
                            <span v-else class="text-xs text-muted-foreground/50">Auto</span>
                        </div>
                    </div>
                </div><!-- end ROW 3 grid -->
                ...
                <div>
                    <div class="mb-1 flex items-center justify-between">
                        <label class="text-xs font-semibold text-foreground">Remarks</label>
                    </div>
                    <textarea
                        v-model="form.remarks"
                        rows="3"
                        placeholder="Auto-generated from selections above..."
                        class="w-full resize-none rounded-lg border border-input bg-background px-3 py-2 text-sm placeholder:text-muted-foreground/50 focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20"
            ...
            <!-- Footer -->
            <div class="flex shrink-0 items-center justify-between border-t border-border/50 bg-muted/10 px-6 py-4">
                <p class="text-[10px] text-muted-foreground/50">
                    <kbd class="rounded border border-border bg-muted px-1 py-0.5 font-mono text-[10px]">Alt+S</kbd> to save
                </p>
